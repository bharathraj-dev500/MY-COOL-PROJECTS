...
while running:
    ...
    if road_y >= HEIGHT:
        road_y = 0
    draw.rect(screen, ROAD_COLOR, (200, road_y, 400, HEIGHT))
    draw.rect(screen, ROAD_COLOR, (200, road_y - HEIGHT, 400, HEIGHT))
    ...
    if keys[K_LEFT] and player_x > 200:
        player_x -= player_speed
    if keys[K_RIGHT] and player_x < 540:  # 800 - 260 to keep within road
        player_x += player_speed
    ...
    for obstacle in obstacles[:]:
        ...
        obs_rect = Rect(obstacle[0], obstacle[1], 40, 40)
        draw.rect(screen, OBSTACLE_COLOR, obs_rect)
    ...
    for pickup in pickups[:]:
        pickup[1] += 3
        pick_rect = Rect(pickup[0]-15, pickup[1]-15, 30, 30)
        draw.ellipse(screen, PICKUP_COLOR, pick_rect)
        if pickup[1] > HEIGHT:
            pickups.remove(pickup)
    
    # Create player rectangle and draw
    player_rect = Rect(player_x, player_y, 60, 30)
    draw.rect(screen, PLAYER_COLOR, player_rect)
